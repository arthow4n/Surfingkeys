...
function createDefaultMappings() {
  ...
  mapkey("yc", "#7Copy a column of a table", function () {
    Hints.create(getTableColumnHeads(), function (element) {
      var column = Array.from(
        element.closest("table").querySelectorAll("tr")
      ).map(function (tr) {
  ...
  mapkey("i", "#1Go to edit box", function () {
    Hints.create(
      "input, textarea, *[contenteditable=true], *[role=textbox], select, div.ace_cursor",
  ...
  mapkey(";s", "Toggle PDF viewer from SurfingKeys", function () {
    ...
    if (
      pdfUrl.indexOf(chrome.extension.getURL("/pages/pdf_viewer.html")) === 0
    ) {
      ...
      if (
        ...
        document.querySelector("EMBED").getAttribute("type") ===
          "application/pdf"
      ) {
  ...
  mapkey(
    "<Alt-i>",
    "#0enter PassThrough mode to temporarily suppress SurfingKeys",
    function () {
      Normal.passThrough();
    }
  );
  ...
  mapkey("Q", "#8Open omnibar for word translation", function () {
    Front.openOmniquery({
      query: Normal.getWordUnderCursor(),
      style: "opacity: 0.8;",
    });
  });
          ...
          element.querySelectorAll("input, textarea").forEach(function (ip) {
            if (fd.hasOwnProperty(ip.name) && ip.type !== "hidden") {
              if (ip.type === "radio") {
                ...
                element
                  .querySelectorAll(`input[name='${ip.name}']`)
                  .forEach(function (ip) {
                    ip.checked = false;
                  });
                var vals = fd[ip.name];
  ...
  function openGoogleTranslate() {
    if (window.getSelection().toString()) {
      ...
      tabOpenLink(
        "https://translate.google.com/translate?js=n&sl=auto&tl=zh-CN&u=" +
          window.location.href
      );
    }
  }
  ...
  addSearchAliasX(
    ...
    "s",
    "http://suggestion.baidu.com/su?cb=&wd=",
    function (response) {
