function createHints() {
  ...
  var hintsStyle = createElementWithContent(
    "style",
    `
div {
    position: absolute;
...
[mode=input] div.activeInput {
    ...
    if (ai !== null) {
      ...
      if (Mode.isSpecialKeyOf("<Esc>", event.sk_keyName)) {
        ...
        _lastCreateAttrs.activeInput =
          (_lastCreateAttrs.activeInput + (event.shiftKey ? -1 : 1)) %
          hints.length;
      ...
      if (event.keyCode === KeyboardUtils.keyCodes.backspace) {
        ...
        if (key !== "") {
          if (self.numericHints) {
            ...
            self.characters.toLowerCase().indexOf(key.toLowerCase()) !== -1
          ) {
  ...
  var prefix = "",
    ...
    holder = createElementWithContent("section", "", {
      style: "display: block; opacity: 1;",
    }),
    shiftKey = false;
  //self.characters = 'asdfgqwertzxcvb';
  self.characters = "asdfqwerzxcv";
  ...
  function isCapital(key) {
    return key === key.toUpperCase() && key !== key.toLowerCase(); // in case key is a symbol or special character
  }
  ...
  function refresh() {
    ...
    hints.forEach(function (hint) {
      ...
      if (prefix.length === 0) {
        ...
        setSanitizedContent(
          hint,
          `<span style="opacity: 0.2;">${prefix}</span>` +
            label.substr(prefix.length)
        );
        matches.push(hint);
      } else {
  ...
  function resetHints(evt) {
    ...
    if (found > 0) {
      self.statusLine +=
        " - " + (new Date().getTime() - start) + "ms / " + found;
      Mode.showStatus();
    }
  }
  ...
  self.coordinate = function () {
    ...
    var link = createElementWithContent("div", "A", {
      style: "top: 0; left: 0;",
    });
    holder.prepend(link);
  ...
  function createHintsForTextNode(rxp, attrs) {
    ...
    elements = positions
      .map(function (e) {
        ...
        if (
          runtime.conf.caretViewport &&
          runtime.conf.caretViewport.length === 4
        ) {
  ...
  self.flashPressedLink = function (link) {
    ...
    var flashElem = createElementWithContent("div", "", {
      style:
