function createMode() {
  ...
  self.isSpecialKeyOf = function (specialKey, keyToCheck) {
    return (
      -1 !==
      self.specialKeys[specialKey].indexOf(
        KeyboardUtils.decodeKeystroke(keyToCheck)
      )
    );
  };
  ...
  self.handleMapKey = function (event, onNoMatched) {
    ...
    if (isEscKey && self.finish(this)) {
      ...
      event.sk_stopPropagation =
        !this.map_node.meta.stopPropagation ||
...
function createNormal() {
  ...
  self.getWordUnderCursor = function () {
    ...
    if (selection.focusNode && selection.focusNode.textContent) {
      var range = getNearestWord(
        selection.focusNode.textContent,
        selection.focusOffset
      );
      var selRect = getTextRect(
        selection.focusNode,
        range[0],
        range[0] + range[1]
      );
      ...
      if (
        selRect &&
        _clickPos &&
        selRect.has(_clickPos[0], _clickPos[1], 0, 0) &&
        word
      ) {
        return word.trim();
      }
  ...
  self.toggleBlocklist = function () {
    if (document.location.href.indexOf(chrome.extension.getURL("")) !== 0) {
      RUNTIME(
        "toggleBlocklist",
        {
          blocklistPattern: runtime.conf.blocklistPattern
            ? runtime.conf.blocklistPattern.toJSON()
            : "",
        },
        function (resp) {
          if (resp.disabled) {
      ...
      Front.showBanner(
        "You could not toggle Surfingkeys on its own pages.",
        3000
      );
    }
  };
  ...
  function initScroll(elm) {
    elm.skScrollBy = function (x, y) {
      if (
        ...
        if (
          (this.scrollTop === 0 && y <= 0 && previousPage()) ||
        ...
        function step(t) {
          if (previousTimestamp === 0) {
            ...
            document.dispatchEvent(
              new CustomEvent("surfingkeys:scrollStarted")
            );
            return window.requestAnimationFrame(step);
          }
          var old = elm[prop],
            delta = ((t - previousTimestamp) * distance) / duration;
  ...
  function scrollableMousedownHandler(e) {
    ...
    for (var i = scrollNodes.length - 1; i >= 0 && index === -1; i--) {
      if (
        scrollNodes[i] !== document.body &&
        scrollNodes[i].contains(e.target)
      ) {
        index = i;
      }
    }
  ...
  self.scroll = function (type) {
    ...
    switch (type) {
      ...
      case "byRatio":
        var y =
          parseInt((RUNTIME.repeats * scrollNode.scrollHeight) / 100) -
          size[1] / 2 -
  ...
  self.addScrollableElement = function (elm) {
    if (
      !scrollNodes ||
      (!elm.contains(scrollNodes[scrollIndex]) &&
        scrollNodes.indexOf(elm) === -1)
    ) {
      initScrollIndex();
      ...
      img.onload = function () {
        ...
        if (lastScrollTop === elm.scrollTop) {
          if (lastScrollLeft === elm.scrollLeft) {
            ...
            Front.showPopup(
              "<img src='{0}' />".format(canvas.toDataURL("image/png"))
            );
            // restore overflow
