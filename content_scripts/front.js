function createFront() {
  ...
  var frameElement = createElementWithContent("div", "Hi, I'm here now!", {
    id: "sk_frame",
  });
  ...
  function getAllAnnotations() {
    return [Normal.mappings, Visual.mappings, Insert.mappings]
      .map(getAnnotations)
      .reduce(function (a, b) {
        return a.concat(b);
      });
  ...
  function updateElementBehindEditor(data) {
    ...
    if (elementBehindEditor.nodeName === "DIV") {
      ...
      if (
        typeof elementBehindEditor.setValueWithEventDispatched === "function"
      ) {
  ...
  function showAceVimEditor(element, onWrite, type) {
    ...
    if (typeof element === "string") {
      ...
      content = Array.from(element.querySelectorAll("option"))
        .map(function (n, i) {
          if (n.value === selected) {
            initial_line = i;
          }
  ...
  self.performInlineQuery = function (query, pos, showQueryResult) {
    if (document.dictEnabled !== undefined) {
      ...
      httpRequest(
        {
          url:
            typeof _inlineQuery.url === "function"
              ? _inlineQuery.url(query)
              : _inlineQuery.url + query,
  ...
  _actions["nextEdit"] = function (response) {
    var sel =
      Hints.getSelector() || "input, textarea, *[contenteditable=true], select";
    sel = getElements(sel);
    if (sel.length) {
  ...
  _actions["getBackFocus"] = function (response) {
    ...
    if (
      window === top &&
      window.uiHost &&
      window.uiHost.shadowRoot.contains(document.activeElement)
    ) {
    ...
    function (event) {
      ...
      if (_message.action === "performInlineQuery") {
        ...
        if (_message.responseToContent && _callbacks[_message.id]) {
          ...
          _message.commandToContent &&
          _message.action &&
          _actions.hasOwnProperty(_message.action)
        ) {
